from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="AnalysisAmazonRekognitionSettingsSchema")


@_attrs_define
class AnalysisAmazonRekognitionSettingsSchema:
    """
    Attributes:
        access_key (str):
        bucket (str):
        endpoint (str):
        path (str):
        region (str):
        secret_key (str):
        is_system (bool | None | Unset):
    """

    access_key: str
    bucket: str
    endpoint: str
    path: str
    region: str
    secret_key: str
    is_system: bool | None | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        access_key = self.access_key

        bucket = self.bucket

        endpoint = self.endpoint

        path = self.path

        region = self.region

        secret_key = self.secret_key

        is_system: bool | None | Unset
        if isinstance(self.is_system, Unset):
            is_system = UNSET
        else:
            is_system = self.is_system

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "access_key": access_key,
                "bucket": bucket,
                "endpoint": endpoint,
                "path": path,
                "region": region,
                "secret_key": secret_key,
            }
        )
        if is_system is not UNSET:
            field_dict["is_system"] = is_system

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        access_key = d.pop("access_key")

        bucket = d.pop("bucket")

        endpoint = d.pop("endpoint")

        path = d.pop("path")

        region = d.pop("region")

        secret_key = d.pop("secret_key")

        def _parse_is_system(data: object) -> bool | None | Unset:
            if data is None:
                return data
            if isinstance(data, Unset):
                return data
            return cast(bool | None | Unset, data)

        is_system = _parse_is_system(d.pop("is_system", UNSET))

        analysis_amazon_rekognition_settings_schema = cls(
            access_key=access_key,
            bucket=bucket,
            endpoint=endpoint,
            path=path,
            region=region,
            secret_key=secret_key,
            is_system=is_system,
        )

        analysis_amazon_rekognition_settings_schema.additional_properties = d
        return analysis_amazon_rekognition_settings_schema

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
