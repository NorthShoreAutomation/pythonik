from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, cast
from uuid import UUID

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="ReindexAllSegmentsSchema")


@_attrs_define
class ReindexAllSegmentsSchema:
    """
    Attributes:
        asset_ids (list[UUID] | None):
        ignore_comments (bool | Unset):
        realms (list[str] | None | Unset):
        sync_to_another_dc (bool | Unset):
    """

    asset_ids: list[UUID] | None
    ignore_comments: bool | Unset = UNSET
    realms: list[str] | None | Unset = UNSET
    sync_to_another_dc: bool | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        asset_ids: list[str] | None
        if isinstance(self.asset_ids, list):
            asset_ids = []
            for asset_ids_type_0_item_data in self.asset_ids:
                asset_ids_type_0_item = str(asset_ids_type_0_item_data)
                asset_ids.append(asset_ids_type_0_item)

        else:
            asset_ids = self.asset_ids

        ignore_comments = self.ignore_comments

        realms: list[str] | None | Unset
        if isinstance(self.realms, Unset):
            realms = UNSET
        elif isinstance(self.realms, list):
            realms = self.realms

        else:
            realms = self.realms

        sync_to_another_dc = self.sync_to_another_dc

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "asset_ids": asset_ids,
            }
        )
        if ignore_comments is not UNSET:
            field_dict["ignore_comments"] = ignore_comments
        if realms is not UNSET:
            field_dict["realms"] = realms
        if sync_to_another_dc is not UNSET:
            field_dict["sync_to_another_dc"] = sync_to_another_dc

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)

        def _parse_asset_ids(data: object) -> list[UUID] | None:
            if data is None:
                return data
            try:
                if not isinstance(data, list):
                    raise TypeError()
                asset_ids_type_0 = []
                _asset_ids_type_0 = data
                for asset_ids_type_0_item_data in _asset_ids_type_0:
                    asset_ids_type_0_item = UUID(asset_ids_type_0_item_data)

                    asset_ids_type_0.append(asset_ids_type_0_item)

                return asset_ids_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(list[UUID] | None, data)

        asset_ids = _parse_asset_ids(d.pop("asset_ids"))

        ignore_comments = d.pop("ignore_comments", UNSET)

        def _parse_realms(data: object) -> list[str] | None | Unset:
            if data is None:
                return data
            if isinstance(data, Unset):
                return data
            try:
                if not isinstance(data, list):
                    raise TypeError()
                realms_type_0 = cast(list[str], data)

                return realms_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(list[str] | None | Unset, data)

        realms = _parse_realms(d.pop("realms", UNSET))

        sync_to_another_dc = d.pop("sync_to_another_dc", UNSET)

        reindex_all_segments_schema = cls(
            asset_ids=asset_ids,
            ignore_comments=ignore_comments,
            realms=realms,
            sync_to_another_dc=sync_to_another_dc,
        )

        reindex_all_segments_schema.additional_properties = d
        return reindex_all_segments_schema

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
